$(function () {
    ...
    $('.cnpj_search').blur(function () {
        ...
        var validate_cnpj = /^[0-9]{14}$/;

        if (cnpj != "" && validate_cnpj.test(cnpj)) {
            ...
            $.getJSON("https://brasilapi.com.br/api/cnpj/v1/" + cnpj, function (data) {
                if (!("erro" in data)) {
                    $("#alias_name").val(data.nome_fantasia);
                    $("#social_name").val(data.razao_social);
    ...
    $('input[type="checkbox"][name="physics"]').change(function(){
        if($(this).get(0).checked) {
            $('input[type="checkbox"][name="legal"]').removeAttr("checked");
        }
    });
